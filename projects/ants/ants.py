...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place=self.place
        i=0
        while place != hive and place !=None and place.entrance != None:
            if len(place.bees)>0 and i>=self.min_range and i <=self.max_range:
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        colony.food+=1
        for bee in b:
            if self.ant!=None: #If tank contains another ant, the other ant attacks too
                bee.reduce_armor(2)
            else:
                bee.reduce_armor(1)
        # END Problem 8
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 9
        # Function that goes backward through each of the places behind the queen
        #Saves and appends doubled damage values
        def backward(move_back):
            present_spot=self.place
            while present_spot != None:
                if present_spot.ant!=None and present_spot.ant not in QueenAnt.stronger:
                    if present_spot.ant.name!="Queen":
                        present_spot.ant.damage*=2
                    if present_spot.ant.container==True and present_spot.ant.ant!=None and present_spot.ant.ant not in QueenAnt.stronger:
                        present_spot.ant.ant.damage*=2

                    QueenAnt.stronger.append(present_spot.ant)
                if move_back:
                    present_spot=present_spot.exit
        #Eliminate any fake queens
        if self.num !=1:
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 9
        self.armor=Insect.reduce_armor(self,amount)
        #End the game if the true queen dies
        if self.num==1 and  self.armor != None and self.armor<=0:
            bees_win()
