...
def accumulate(combiner, base, n, term):
    ...
    for i in range(1,n+1):
        total,i=combiner(total,term(i)),increment(i)
    return total
...
def filtered_accumulate(combiner, base, pred, n, term):
    ...
    def combine_if(x, y):
        if pred(x)==True:
            return combiner(x,y)
        elif pred(x in range(n+1))==False:
            return base
        else:
            return y
...
def repeated(f, n):
    ...
    """
    if n==0:
        return lambda x:accumulate(compose1,x,0,f)
    else:
        return lambda x:accumulate(compose1,x,n,f)(x)
