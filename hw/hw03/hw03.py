...
def pingpong(n):
    ...
    def change(k,s,add):
        if has_seven(k)==True or k%7==0:
            return sequence(k,s, not add)
        else:
            return sequence(k,s,add)
    def sequence(k,s,add):
        if k==n:
            return s
        if add:
            return change(k+1,s+1,add)
        else:
            return change(k+1,s-1,add)
    return sequence(1,1,True)
...
def deep_len(lnk):
    ...
    5
    """
    return print_link(rest(rest(lnk)))
...
def riffle(deck):
    ...
    """
    assert len(deck)%2==0
    a=flatten([[i,len(deck)//2+i] for i in deck])
    return a[0:len(deck)]
...
def build_binary_ones(h):
    ...
    if h==0:
        ...
        return tree(1,[tree(1),build_binary_ones(h-1)])
